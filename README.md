...
- 正式后端交易：支持购物车、创建订单、支付模拟、支付/退款流水、会员积分账本、管理员发货、用户收货、评价、取消、申请退单、审核退单。
- 正式后端智能模块：支持附近门店查询、RAG-lite 智能问答、Item-CF + 马尔可夫链 + 热度 + LBS 的混合推荐。
...
- 用户端：`frontend/user-app`，Flutter 项目，预留 Web 与 Android 平台，支持登录、首页、推荐、附近门店、商品、购物车、订单和 AI 助手页面。
- 管理端：`frontend/admin-app`，Flutter 项目，预留 Web 与 Android 平台，支持登录、数据看板、会员、商品、订单、宠物审核、退款审核、门店和媒体管理页面。
...
- 支持附近宠物商店搜索、宠物/周边商品浏览、购物车、下单、支付模拟、支付流水、会员积分、发货、收货、评价、取消订单、申请退单、管理员审核退单。
