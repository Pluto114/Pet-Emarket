...
- AI 联调：Java 后端 `/api/v1/ai/chat` 已预留独立 FastAPI AI 服务网关，可通过 `pet-emarket.ai-service.enabled=true` 转发到 `ai-recommendation-service`。
...
# 2. 建表 + 种子数据
cd Pet-Emarket
docker exec -i mysql-local mysql -u root -p123456 pet_emarket < database/mysql/migrations/001_init_schema.sql
docker exec -i mysql-local mysql -u root -p123456 pet_emarket < database/mysql/migrations/002_order_state_log.sql
docker exec -i mysql-local mysql -u root -p123456 pet_emarket < database/mysql/seeds/seed_demo_data.sql

# 3. 验证
docker exec mysql-local mysql -u root -p123456 pet_emarket -e "SHOW TABLES;"
```

13 张表含 2 个用户 + 6 个商品 + 12 条行为训练数据。
