  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(padding: const EdgeInsets.all(20), children: [
        Text('订单管理', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
        const SizedBox(height: 12),
        if (loading) const Center(child: Padding(padding: EdgeInsets.all(28), child: CircularProgressIndicator())),
        if (!loading && orders.isEmpty) const Card(child: Padding(padding: EdgeInsets.all(20), child: Text('暂无订单'))),
        ...orders.map((o) => Card(
