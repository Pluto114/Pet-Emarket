  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(padding: const EdgeInsets.all(20), children: [
        Text('活体宠物审核', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
        const SizedBox(height: 12),
        if (loading) const Center(child: Padding(padding: EdgeInsets.all(28), child: CircularProgressIndicator())),
        if (!loading && livePets.isEmpty) const Card(child: Padding(padding: EdgeInsets.all(20), child: Text('暂无活体宠物'))),
        ...livePets.map((p) => Card(
