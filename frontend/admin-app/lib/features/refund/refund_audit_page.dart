...
class _RefundAuditPageState extends State<RefundAuditPage> {
  ...
  Future<void> _audit(PetOrder o, bool approved) async {
    try {
      await widget.apiClient.operateOrder(o.id, 'audit-refund', body: {'approved': approved, 'auditRemark': approved ? '审核通过' : '审核拒绝'});
      await load();
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(padding: const EdgeInsets.all(20), children: [
        Text('退单审核', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
        const SizedBox(height: 12),
        if (loading) const Center(child: Padding(padding: EdgeInsets.all(28), child: CircularProgressIndicator())),
        if (!loading && refundOrders.isEmpty) const Card(child: Padding(padding: EdgeInsets.all(20), child: Text('暂无待审核退单'))),
        ...refundOrders.map((o) => Card(
          child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              OutlinedButton(onPressed: () => _audit(o, false), style: OutlinedButton.styleFrom(foregroundColor: Colors.red), child: const Text('拒绝退单')),
            ]),
          ])),
        )),
      ]),
