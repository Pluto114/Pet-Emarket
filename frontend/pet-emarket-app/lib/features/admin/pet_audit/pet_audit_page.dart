  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(padding: const EdgeInsets.all(20), children: [
        Text('宠物审核', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
        const SizedBox(height: 12),
        if (loading) const Center(child: Padding(padding: EdgeInsets.all(28), child: CircularProgressIndicator())),
        if (!loading && livePets.isEmpty) const Card(child: Padding(padding: EdgeInsets.all(20), child: Text('暂无待审核宠物'))),
        ...livePets.map((p) => Card(
          child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (p.auditStatus == 'PENDING' || p.status == 'DRAFT') ...[
              const SizedBox(height: 8),
              FilledButton(onPressed: () => _approve(p), child: const Text('审核通过并上架')),
            ],
          ])),
        )),
      ]),
