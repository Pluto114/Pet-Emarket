...
import '../../../../core/theme/app_theme.dart' show radiusCard, voldogOrange;
...
class _HS extends State<HomePage> with TickerProviderStateMixin {
  ...
  @override void dispose(){_breathCtrl.dispose();_timer.cancel();_pageCtrl.dispose();_sc.dispose();super.dispose();}

  @override Widget build(BuildContext ctx){
    final t=Theme.of(ctx);final s=t.colorScheme;final u=widget.sessionStore.user;
    return Scaffold(backgroundColor:s.surface,
      body:Stack(children:[
        CustomScrollView(physics:const BouncingScrollPhysics(),slivers:[
          ...
          SliverAppBar(floating:true,pinned:false,snap:true,backgroundColor:s.surface,surfaceTintColor:Colors.transparent,elevation:0,toolbarHeight:72,titleSpacing:16,
            flexibleSpace:SafeArea(child:Padding(padding:const EdgeInsets.symmetric(horizontal:16,vertical:8),child:Row(children:[
              Expanded(child:Container(height:48,decoration:BoxDecoration(color:s.surfaceContainerLow,borderRadius:BorderRadius.circular(24),boxShadow:[BoxShadow(color:s.primary.withAlpha(12),blurRadius:10,offset:const Offset(0,3))]),child:TextField(controller:_sc,decoration:InputDecoration(hintText:'搜索宠物、口粮…',hintStyle:TextStyle(color:s.onSurface.withAlpha(80),fontSize:14),prefixIcon:Icon(Icons.search_rounded,color:s.onSurface.withAlpha(100)),border:InputBorder.none,contentPadding:const EdgeInsets.symmetric(vertical:13))))),
          ...
          //── Banner ──
          SliverToBoxAdapter(child:SizedBox(height:155,child:PageView.builder(
          ...
          //── 金刚区 ──
          SliverToBoxAdapter(child:Padding(padding:const EdgeInsets.symmetric(vertical:10,horizontal:4),child:Row(mainAxisAlignment:MainAxisAlignment.spaceEvenly,children:_cats.map((c)=>InkWell(onTap:(){},borderRadius:BorderRadius.circular(20),child:SizedBox(width:60,child:Column(children:[
            Container(width:48,height:48,decoration:BoxDecoration(shape:BoxShape.circle,color:s.primaryContainer,boxShadow:[BoxShadow(color:s.primary.withAlpha(30),blurRadius:8,offset:const Offset(0,2))]),child:Icon(c['i']as IconData,color:s.primary,size:24)),
          ...
          SliverPadding(padding:const EdgeInsets.symmetric(horizontal:14),sliver:SliverLayoutBuilder(builder:(_,cstr){
            ...
            return SliverGrid(gridDelegate:SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:cols,childAspectRatio:r,mainAxisSpacing:10,crossAxisSpacing:10),
              delegate:SliverChildBuilderDelegate((_,i)=>_card(_prods[i],i,s),childCount:_prods.length));
          })),
          const SliverToBoxAdapter(child:SizedBox(height:100)),
        ]),
        //── AI 悬浮球 ──
        Positioned(right:18,bottom:24,child:AnimatedBuilder(animation:_breathCtrl,builder:(_,c)=>Transform.scale(scale:_breathAnim.value,child:c),child:GestureDetector(
          onTap:()=>Navigator.push(ctx,MaterialPageRoute(builder:(_)=>AiAssistantPage(apiClient:widget.apiClient))),
          child:Container(width:56,height:56,decoration:BoxDecoration(shape:BoxShape.circle,color:s.primary,boxShadow:[BoxShadow(color:s.primary.withAlpha(80),blurRadius:18,offset:const Offset(0,6))]),child:const Icon(Icons.smart_toy_rounded,color:Colors.white,size:28)),
        ))),
      ]));
  }

  Widget _card(_P p,int i,ColorScheme s){
    return GestureDetector(onTap:(){},child:Container(
      decoration:BoxDecoration(color:s.surfaceContainerLow,borderRadius:BorderRadius.circular(radiusCard),boxShadow:[BoxShadow(color:s.shadow.withAlpha(12),blurRadius:10,offset:const Offset(0,3))]),
      child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        ...
        Expanded(flex:3,child:ClipRRect(borderRadius:const BorderRadius.vertical(top:Radius.circular(radiusCard)),child:Stack(children:[
          ...
          Positioned(left:8,top:8,child:ClipRRect(borderRadius:BorderRadius.circular(10),child:BackdropFilter(filter:ImageFilter.blur(sigmaX:6,sigmaY:6),child:Container(
            ...
            child:Row(mainAxisSize:MainAxisSize.min,children:[const Text('🐾',style:TextStyle(fontSize:10)),const SizedBox(width:3),Text(p.r,style:const TextStyle(fontSize:9,color:Colors.white,fontWeight:FontWeight.w500))]),
          )))),
          // 右下评分
...
class _IB extends StatelessWidget{const _IB({required this.icon,required this.s,this.badge=0,this.onTap});final IconData icon;final ColorScheme s;final int badge;final VoidCallback? onTap;
  @override Widget build(BuildContext c)=>GestureDetector(onTap:onTap,child:Container(width:42,height:42,decoration:BoxDecoration(color:s.surfaceContainerLow,borderRadius:BorderRadius.circular(16)),child:Stack(children:[
...
class _SC extends StatelessWidget{const _SC({required this.rating,required this.s,this.compact=false});final double rating;final ColorScheme s;final bool compact;
  @override Widget build(BuildContext c)=>Container(padding:EdgeInsets.symmetric(horizontal:compact?5:7,vertical:compact?1:2),decoration:BoxDecoration(color:s.primaryContainer,borderRadius:BorderRadius.circular(compact?5:8)),child:Row(mainAxisSize:MainAxisSize.min,children:[Icon(Icons.star_rounded,size:compact?9:11,color:s.primary),const SizedBox(width:1),Text('$rating',style:TextStyle(fontSize:compact?9:11,fontWeight:FontWeight.w700,color:s.primary))]));
}
