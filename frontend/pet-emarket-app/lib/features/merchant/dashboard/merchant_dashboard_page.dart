...
class MerchantDashboardPage extends StatefulWidget {
  const MerchantDashboardPage({required this.apiClient, required this.sessionStore, super.key});
      ...
      child: ListView(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text('商家仪表盘', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
                    const SizedBox(height: 4),
                    Text('欢迎回来，${widget.sessionStore.user?.displayName ?? "商家"}',
                        style: theme.textTheme.bodyMedium?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
                  ],
                ),
              ),
              IconButton.filledTonal(onPressed: loadData, icon: const Icon(Icons.refresh)),
            ],
          ),
            ...
            builder: (ctx, constraints) {
              ...
              return Wrap(
                ...
                children: [
                  ...
                  _StatCard(icon: Icons.pets, label: '活体宠物', value: (data?.livePetCount ?? 0).toString(), color: const Color(0xFF7C4DFF), wide: wide),
                  _StatCard(icon: Icons.store, label: '店铺状态', value: (data?.openStoreCount ?? 0) > 0 ? '营业中' : '已关闭', color: const Color(0xFF204E4A), wide: wide),
                ],
              );
            },
          ),
                ...
                child: Row(
                  children: [
                    ...
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('总销售额', style: theme.textTheme.bodyMedium?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
                        const SizedBox(height: 2),
                        Text('¥${data.totalPayAmount.toStringAsFixed(2)}', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
                      ],
                    ),
                  ],
                ),
          ...
          if (data != null && data.orderStatusDistribution.isNotEmpty) ...[
            Text('订单状态分布', style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w600)),
            const SizedBox(height: 12),
            Card(
              child: Padding(
                padding: const EdgeInsets.all(20),
          ...
          if (data != null && data.topProducts.isNotEmpty) ...[
            Text('热销商品', style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w600)),
            const SizedBox(height: 12),
            ...data.topProducts.map((p) => Card(
              child: ListTile(
                ...
                trailing: Text('¥${p.amount.toStringAsFixed(2)}', style: TextStyle(fontWeight: FontWeight.w700, color: theme.colorScheme.primary)),
              ),
            )),
            const SizedBox(height: 28),
          ],
          // Recent orders
          Text('最近订单', style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w600)),
          const SizedBox(height: 12),
  ...
  List<PieChartSectionData> _buildPieSections(ThemeData theme) {
    ...
    return counts.asMap().entries.map((e) {
      ...
      return PieChartSectionData(
        ...
        title: entry.count > 0 ? entry.count.toString() : '',
        radius: 60,
